...
describe('trimVideo 디스패치', () => {
  ...
  it('URL 소스 → 서버 트리밍(trimVideoServer)', async () => {
    await trimVideo({ inputFile: null, source: 'url', originalUrl: 'https://x/y', startTime: 1, endTime: 2 });
    expect(trimVideoServer).toHaveBeenCalledTimes(1);
    expect(trimVideoServer).toHaveBeenCalledWith(expect.objectContaining({ originalUrl: 'https://x/y' }));
  ...
  it('파일 + 그 외 타입 → FFmpeg + 로드 통지', async () => {
    ...
    await trimVideo({
      inputFile: fakeFile(), source: 'file', startTime: 0, endTime: 5,
      onFFmpegLoadStart, onFFmpegLoadComplete,
    });
  ...
  it('FFmpeg 로드 실패해도 onFFmpegLoadComplete 호출 후 throw', async () => {
    ...
    const onFFmpegLoadComplete = vi.fn();
    await expect(
      trimVideo({ inputFile: fakeFile(), source: 'file', startTime: 0, endTime: 5, onFFmpegLoadComplete })
