...
const ERROR_DEFINITIONS: Record<
  ErrorCode,
  Omit<AppError, "code" | "technicalDetails" | "context">
> = {
  MEMORY_INSUFFICIENT: {
    message: "Insufficient memory",
    userMessage: "메모리가 부족합니다.",
    solution: "더 작은 파일을 사용하거나 브라우저를 재시작해주세요. 권장 파일 크기: 500MB 이하",
  },
  CODEC_UNSUPPORTED: {
  ...
  BINARY_MISSING: {
    ...
    solution:
      "`npm install`을 다시 실행해 의존성 바이너리를 설치하세요(postinstall). 설치 로그를 확인해주세요.",
  },
  TIMEOUT: {
...
export function classifyError(raw: string): ErrorCode {
  ...
  // 실행 파일 없음
  if (
    /enoent|command not found|no such file|설치되어 있지 않|not installed|executable not found|cannot find .*(yt-dlp|ffmpeg|streamlink|aria2)/.test(
      m,
    )
  ) {
  ...
  if (
    /econnrefused|econnreset|enotfound|fetch failed|network|연결할 수 없|connection (refused|reset|lost)|socket hang up|getaddrinfo/.test(
      m,
    )
  ) {
