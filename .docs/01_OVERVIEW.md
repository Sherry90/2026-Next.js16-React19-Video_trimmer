...
**편집**
- 타임라인 에디터: 핸들 드래그, 플레이헤드, 오디오 파형 + 스펙트럴 오버레이(항상 겹침), 줌(0.1x–10x)
...
| **video.js** | 8.x | 비디오 재생 (HLS/DASH 포함), 커스텀 React 컨트롤바 |
| **wavesurfer.js** | 7.x | 오디오 파형 렌더 (스펙트럴 위에 오버레이) |
| **STFT 스펙트럴** | 자체 구현 | FFT 기반 스펙트로그램(`shared/lib/spectrogram.ts`), 파형과 겹쳐 표시 |
...
│   │   └── hooks/           useUrlInput
│   ├── player/              # video.js 플레이어 + 커스텀 React 컨트롤
│   │   ├── components/      VideoPlayerView (player 인스턴스를 context로 노출), VideoScreen,
│   │   │                    PlayerControlBar → PlayerControls → PlayButton, VolumeControl,
│   │   │                    Scrubber, TimeDisplay, FullscreenButton, QualitySelector
...
│   └── lib/                 # 순수 유틸: waveformCache, spectrogram, spectrogramCache, retry,
│                            platformUrl, timeFormatter, mathUtils, stringUtils, formatBytes,
│                            errorHandler, ffmpegLogParser, memoryMonitor
│
├── lib/                     # 서버 전용 유틸리티
...
**파형·스펙트럴 오버레이 (시각 설정 중앙화)**: 타임라인은 스펙트럴(아래) + 반투명 파형(위, `screen` 블렌드)을 **항상 겹쳐** 표시한다(토글 없음). 색/투명도/블렌드/FFT·hop·bin·로그스케일/그리드를 `APP_CONFIG.WAVEFORM_VIEW` 단일 출처에서 조절하며, 로컬(파일)·URL·서버 스펙트럴 분석이 **동일 상수**(`SPECTRAL_SAMPLE_RATE`/`FFT_SIZE`/`HOP_SIZE`/`FREQ_BINS`)를 공유해 시간축이 정합된다.
...
2. 편집 (editing)
   - video.js 초기화, WaveformBackground가 로컬 미디어에서 파형 디코드 + STFT 스펙트럴 계산(겹쳐 표시)
...
1. URL 입력 (idle → editing)
   ...
   - resolve와 병렬로 /api/video/waveform · /api/video/spectrogram prefetch 시작

2. 편집 (editing) — 전체 소스 위 구간 선택
   - HLS: 프록시가 m3u8 내부 세그먼트 URI를 /api/video/proxy 경유로 재작성 (CORS)
   - MP4: /api/video/proxy?url=… 로 Range 패스스루
   - WaveformBackground는 prefetch된 서버 peaks + 스펙트럴 프레임을 소비 (영상 전체 다운로드 없음)
