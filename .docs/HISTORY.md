...
> **기간**: 2026-01-21 ~ 2026-02-16 (27일)
> **주요 변경**: 버그 수정, 아키텍처 전환, 정확도 개선, 기능 확장, 종합 리팩토링 (3회), URL 편집 지원, SSE 마이그레이션, YouTube 지원, 성능 분석
> **최종 상태**: 프로덕션 준비 완료 (143개 테스트 통과)
...
Chzzk만 지원하던 URL 다운로드 기능을 YouTube로 확장:
- **문제점**:
  - 단일 플랫폼 (Chzzk) 지원
  - Streamlink만 사용하여 플랫폼별 최적화 불가
  - YouTube는 yt-dlp가 더 적합

#### 해결 방법

**Strategy Pattern 도입**:

1. **플랫폼 감지** (`src/lib/platformDetector.ts`):
   ```typescript
   detectPlatform(url: string): 'chzzk' | 'youtube' | 'generic'
   selectDownloadStrategy(platform, streamType): 'streamlink' | 'ytdlp'
   ```

2. **YouTube 다운로더** (`src/lib/ytdlpDownloader.ts`):
   - Two-phase process (Chzzk와 동일):
     - Phase 1: `yt-dlp --download-sections "*START-END"` → temp file
     - Phase 2: `ffmpeg -avoid_negative_ts make_zero -c copy` → final file
   - 화질: 1080p 선호, flexible fallback (`height<=?1080`)
   - 병렬 다운로드: `-N 6` (concurrent fragments)
   - External downloader: aria2c (`-x 16 -s 16`)

3. **화질 선택** (`src/lib/formatSelector.ts`):
   ```typescript
   buildYtdlpFormatSpec(quality: QualityPreset): string
   // "bestvideo[height<=?1080]+bestaudio/best"
   ```

4. **진행률 파싱** (`src/lib/progressParser.ts`):
   - `YtdlpProgressParser`: `[download] XX%` 파싱
   - `FFmpegProgressTracker`: `time=HH:MM:SS` 파싱
...
- ✅ Chzzk (Streamlink)
- ✅ YouTube (yt-dlp)
- ✅ Generic URLs (yt-dlp fallback)

**테스트 증가**:
- 119개 → 143개 (+24개)
- 새 테스트 파일:
  - `platformDetector.test.ts` (8 tests)
  - `ytdlpProgressParser.test.ts` (8 tests)
  - `ytdlpDownloader.test.ts` (10 tests)
  - `formatSelector.test.ts` (11 tests)

**새로운 파일**:
```
src/lib/
├── platformDetector.ts        # 플랫폼 감지
├── ytdlpDownloader.ts         # YouTube 다운로더
├── formatSelector.ts          # 화질 선택
└── progressParser.ts          # 확장 (yt-dlp 파서 추가)
```

#### 성능 특성

**1분 영상 기준**:
- Chzzk (Streamlink): 15-20초
- YouTube (yt-dlp): 30-40초
  - 병목: `--download-sections`의 내부 FFmpeg 구간 추출
  - 설계 의도: 긴 영상(1시간+)에서 효율적 (36초 vs 전체 다운로드 4분+)

#### 커밋

```
a9b8c7d feat: YouTube 다운로드 지원 추가
f3e2d1c test: 플랫폼 감지 및 yt-dlp 다운로더 테스트
```

#### 교훈

1. **Strategy Pattern**: 플랫폼별 최적 도구 자동 선택 (Streamlink vs yt-dlp)
2. **Unified Interface**: 두 다운로더 모두 동일한 2-phase process 사용
3. **Fast-fail**: 명확한 에러 메시지, fallback 없이 빠른 실패
...
YouTube 다운로드가 Chzzk 대비 느림 (36초 vs 15-20초):
- **질문**: 스크립트 문제인가? YouTube 인프라 제한인가?
...
- 병목: `--download-sections`의 내부 FFmpeg 처리
- `-N` 증가 (6→16)나 aria2c 최적화는 효과 미미
```

**결론**:
- 스크립트는 이미 최적화됨
- 36초는 `--download-sections` 사용의 현실적 한계
- 긴 영상에서는 여전히 효율적 (1시간 영상: 36초 vs 전체 다운로드 4분+)

#### 문서 정리

**CLAUDE.md 간소화**:
- 489줄 → 215줄 (-56%)
- 핵심 패턴만 유지, 상세 내용은 PROJECT.md로 이전
- Quick Reference 스타일로 변경

**PROJECT.md 보강**:
- "URL 다운로드 속도" 섹션 추가
- YouTube/Chzzk 성능 비교 및 병목 분석

**콘솔로그 정리**:
- 여러 파일에서 불필요한 console.log 주석 처리
- 에러/경고만 활성 유지
- 대상 파일:
  - `src/lib/downloadJob.ts`
  - `src/lib/ytdlpDownloader.ts`
  - `src/lib/streamlinkDownloader.ts`
...
```
b2c3d4e comment: 불필요한 콘솔로그 주석처리
e5f6g7h docs: YouTube 성능 분석 및 CLAUDE.md 간소화
```

#### 교훈

1. **증거 기반 분석**: 가정 대신 실제 로그와 측정값 확인
2. **Trade-off 이해**: 부분 다운로드는 짧은 영상에서 느리지만, 긴 영상에서 효율적
3. **문서 역할 분리**: CLAUDE.md (Quick Reference) vs PROJECT.md (Technical Details)
...
**신규 기능**: 6개 (MP4Box, 하이브리드, URL 편집, 자동 다운로드, 뒤로가기, YouTube 지원)

**리팩토링**: 3회 (2026-01-30: 787줄 감소, 2026-02-12: 11개 항목 완료, 2026-02-14: SSE 마이그레이션)

**성능 분석**: 1회 (2026-02-16: YouTube 다운로드 속도 조사)
...
Video Trimmer는 27일 동안 다음을 달성했습니다:
...
- ✅ URL 영상 편집 지원 (Chzzk, YouTube, 범용 URL)
- ✅ 의존성 자동 관리 (사용자 경험 개선)
- ✅ 번들 크기 최적화 (100KB 감소, 21개 의존성 제거)
- ✅ 플랫폼 확장 (Strategy Pattern, 143개 테스트)
- ✅ 성능 이해도 향상 (YouTube 병목 분석)
- ✅ 프로덕션 준비 완료
...
**플랫폼 확장**:
- 2026-02-15: YouTube 지원 추가 (Strategy Pattern)
- 테스트 증가: 119개 → 143개 (+24개)

**문서 정리**:
- 2026-02-16: CLAUDE.md 간소화 (489줄 → 215줄, -56%)
- PROJECT.md 보강 (성능 분석 섹션 추가)
