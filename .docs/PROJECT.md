...
| **YouTube** | 30-40초 | yt-dlp (구간 다운로드) | FFmpeg 구간 추출 |

**YouTube 성능 상세** (2026-02-16 분석 및 실측):
- 총 시간: ~36초 (네트워크 다운로드 + FFmpeg 구간 추출의 총합)
- 병목: `--download-sections`의 내부 FFmpeg 구간 추출 과정
- 네트워크 속도:
  - 구간 다운로드: **약 6 Mbps** (실측: 30초 1080p 구간 12.2 MB를 16초에 다운로드)
  - 속도 차이 이유: 구간 다운로드는 네트워크와 FFmpeg 처리가 동시 진행되어 throughput 감소
- 최적화 한계: `-N` 값 증가나 aria2c 연결 수 조정은 효과 미미
